...
// Slightly quicker & higher pitch to sound more "young" / energetic.
const SPEECH_RATE = 1.02;
const TYPING_MS_NO_SPEECH = 48;
const UTTERANCE_PITCH = 1.08;
...
function EndCrackersOverlay({ show }: { show: boolean }) {
  // Exactly 3 blasts at the end (bottom -> top via `pirates-cracker-rise`).
  const items = useMemo(
    () => [
      { key: 0, left: '22%', delay: '0s', dur: '1.25s', drift: '-28px', sym: '🎆' },
      { key: 1, left: '50%', delay: '0.15s', dur: '1.25s', drift: '0px', sym: '💥' },
      { key: 2, left: '78%', delay: '0.3s', dur: '1.25s', drift: '28px', sym: '✨' },
    ],
    [],
  );
      ...
      {isFinal && (
        <div className="absolute inset-0 z-[45] flex flex-col">
          <div className="flex-1 flex items-center justify-center px-4">
            <div className="text-center text-[clamp(44px,9vw,76px)] font-extrabold text-amber-200 tracking-tight">
              Go Pirates!
            </div>
          </div>
