...
export default async function MatchMediaDetailPage({ params }: { params: Promise<{ matchId: string }> }) {
  ...
  let mediaList: { id: string; type: string; url: string; title: string | null }[] = [];
  ...
  return (
    <div>
      <Link href="/media" className="text-amber-400 hover:underline text-sm mb-4 inline-block">← Back to Match Media</Link>
      <h2 className="text-2xl font-bold text-pirate-gold mb-2">{format(matchDate, 'MMMM d')} Match</h2>
      <p className="text-slate-400 mb-6">vs {match.opponent}</p>
      ...
      <div className="grid md:grid-cols-2 gap-8 mt-8">
        ...
        <section className="card">
          ...
          <p className="text-slate-500 text-sm">Click Videos to open the album.</p>
        </section>
      </div>
    </div>
  );
